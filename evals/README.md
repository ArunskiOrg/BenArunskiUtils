...
1. Check out the branch under test, then install the skills from that checkout rather than from the published default branch, since `npx skills add ArunskiOrg/BenArunskiUtils` would fetch `main` and not the branch you are testing. Delete the destinations first, then copy, so a repeat run starts from the branch's state and not a merge of it with the previous run: `rm -rf ~/.claude/skills/tts ~/.claude/skills/explain-yourself && cp -r skills/tts skills/explain-yourself ~/.claude/skills/` on macOS and Linux, or `Remove-Item -Recurse -Force "$env:USERPROFILE/.claude/skills/tts", "$env:USERPROFILE/.claude/skills/explain-yourself" -ErrorAction SilentlyContinue; Copy-Item -Recurse skills/tts, skills/explain-yourself "$env:USERPROFILE/.claude/skills/"` on Windows. Copying without deleting first nests the folder on POSIX and merges on Windows, and a `.bootstrap-verified` left behind by an earlier run puts `tts-03` in the wrong state without failing anything. For the no-skill half of a baseline run, stop after the delete.
